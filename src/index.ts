...
import EraserTailCloud from "./cloud";
import * as ET from "./types";
const shader = require('shader');
require('dotenv').config()
export class EraserTailClient{
    public name:string;
    public nameHuman:string;
    private prefix:string;
    public styles:ET.ERASERTAIL_LOGGING_STYLE[];
    private colorPrimary:string;
    private colorSecondary:string;
    private cloudEnabled:boolean;
    private autoHeartbeatEnabled:boolean;
    private autoHeartbeatInterval:number|null;
    public applicationIcon:string|null;
    public services:ET.ERASERTAIL_HEARTBEAT_SERVICE[] = [];
    constructor(params:ET.ERASERTAIL_CLIENT_PARAMS){
        ...
        this.styles = params.APPLICATION_LOGGING_STYLES??[]
        ...
        this.applicationIcon = params.APPLICATION_ICON??"https://www.notion.so/image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2Fea120301-dd55-42ac-9bae-550b36204722%2Ffirefox_duIP1WdCE3.png?table=collection&id=8e076208-75be-401e-ad0f-79b09609b69f&spaceId=1a8c758f-4957-4029-8be0-91f00204b052&width=60&userId=297acf67-47ed-4477-9548-7e39beb85ca2&cache=v2"
        ...
        if(params.LOG_TO_CLOUD &&(typeof process.env.DATADOG_API_KEY !== "string")) this.log("EraserTail Warn", "Unable to connect to EraserTail Cloud: DATADOG_API_KEY environment variable is undefined.")
    ...
    runAutoHeartbeat(){
        ...
        setTimeout(() => {
                ...
                this.runAutoHeartbeat()
        }, this.autoHeartbeatInterval??30000) // If not provided, push heartbeat every 30 seconds.
        
    }
    updateService(serviceName:ET.ERASERTAIL_HEARTBEAT_SERVICE["SERVICE_NAME"], serviceStatus:ET.ERASERTAIL_HEARTBEAT_SERVICE["SERVICE_STATUS"]){
        const serviceIndex = this.services.findIndex((service: ET.ERASERTAIL_HEARTBEAT_SERVICE) => service.SERVICE_NAME === serviceName);
        this.services[serviceIndex].SERVICE_STATUS = serviceStatus;
        return this.services[serviceIndex].SERVICE_STATUS;
    }
    log(type:ET.ERASERTAIL_LOG["LOG_TYPE"], output:string|object, description:string|object="NoneProvided", cloud:boolean=true, verboseInConsole:boolean=false){
        if(description==="NoneProvided" && type === "Heartbeat") description = {
            SERVICES: this.services
        }
        const logStyleDetails = this.styles.find((logStyle) => logStyle.LOG_TYPE === type) as ET.ERASERTAIL_LOGGING_STYLE;
        ...
        if(logStyleDetails.STYLE_PREFIX_BOLD) prefixStyle = prefixStyle.bold
        if(logStyleDetails.STYLE_LINE_BOLD) lineStyle = lineStyle.bold
        ...
        if(this.cloudEnabled && cloud){
            const ETCloudClient = new EraserTailCloud(this)
            ETCloudClient.logs.create({
                ...
                HEARTBEAT: type==="Heartbeat"?description as ET.ERASERTAIL_HEARTBEAT:null
            })
        }
    }
}